...
const data = {
    data: new SlashCommandBuilder()
		.setName('twitch')
		.setDescription('La commande permet de faire des actions liées à twitch')
			...
			subcommand
				...
				.addStringOption(option => 
					option
						...
						.setRequired(true)
						.addChoices(
							{ name: 'Chaîne twitch', value: 'channel' },
							{ name: 'Clip twitch', value: 'video' }
						))
				.addStringOption(option => 
					option
						.setName('nom')
...
const SubExecute = {
	add: async (interaction) => {
		...
		try {
			...
			if (type === "channel") {
				json[name].channelTwitch.push(url);
				fs.writeFileSync("./data/" + "accounts.json", JSON.stringify(json), 'utf8')
				await interaction.reply("✅ chaîne twitch ajouté sur le compte tiktok")
			} else {
        		var account = json[name]

				if (!account)
					return await interaction.reply("❌ Erreur le compte tiktok n'existe pas")
				twitchEditor(interaction.client, account, url)
